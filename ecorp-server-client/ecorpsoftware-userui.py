   ...
   def __init__(self, *args, **kwargs):
    ...
    #   self.left = ImageTk.PhotoImage(file="left-logo.png")
     #  left = tk.Label(self, image=self.left).place(x=60, y=80, width=335, height=303)
       ...
       self.left = ImageTk.PhotoImage(file="images/clientui/user-center-logo.png")
       left = tk.Label(self, image=self.left).place(relx=0.5, rely=0.4, width=335, height=303,anchor='s')

       ctr_panel = tk.Label(self, text="Applications", font=("arial", 15, "bold"), bg="#82defb", fg="white").place(x=1090,y=0)



       self.computerborder=ImageTk.PhotoImage(file="images/clientui/computerborder2.png")
       computerborder = tk.Label(self, image=self.computerborder).place(x=1000, y=30, width=300, height=172)

       self.noteepad=ImageTk.PhotoImage(file="images/clientui/notepad.png")
       noteepad= tk.Button(self, image=self.noteepad,cursor="hand2", highlightthickness=0, bd=0, command=open_apps_notepad).place(x=1240, y=40, width=40, height=37)

       self.firefox = ImageTk.PhotoImage(file="images/clientui/firefox1.png")
       firefox = tk.Button(self, image=self.firefox, cursor="hand2", highlightthickness=0, bd=0,command=open_apps_firefox).place(x=1200, y=40, width=40, height=37)

       self.zoom = ImageTk.PhotoImage(file="images/clientui/zoom1.png")
       zoom = tk.Button(self, image=self.zoom, cursor="hand2", highlightthickness=0, bd=0,).place(x=1160, y=40, width=40, height=37)

       self.filex = ImageTk.PhotoImage(file="images/clientui/explorer1.png")
       filex = tk.Button(self, image=self.filex, cursor="hand2", highlightthickness=0, bd=0,command=open_apps_explorer).place(x=1120, y=40, width=40, height=37)
